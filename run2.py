...
def main():
    ...
    subject_train = subject_data[:(17 * 10) + 1]
    train_label_0 = all_labels[:(17 * 10) + 1]

    # Extract one augmented data from the next 8 subjects for each block of 23 subjects
    subject_val = []
    labels_val = []
    for i in range(170, 251):
        subject_val.append(subject_data[i])
        labels_val.append(all_labels[i])

    subject_train.extend(subject_data[251:(42 * 10)+1])
    train_label_1 = all_labels[251:(42 * 10)+1]

    for i in range(420, 481):
        subject_val.append(subject_data[i])
        labels_val.append(all_labels[i])

    # subject_train.extend(subject_data[481:(65 * 10)+1])
    # train_label_2 = all_labels[481:(65 * 10)+1]

    # for i in range(651, 710):
    #     subject_val.append(subject_data[i])
    #     labels_val.append(all_labels[i])

    labels_train = np.concatenate((train_label_0, train_label_1), axis=0)

    subject_train,labels_train=shuffle(subject_train,labels_train,random_state=42)

    subject_val, subject_test, labels_val, labels_test = train_test_split(
    ...
    model = GCN(hidden_channels=84, number_of_features=feature_dimensions, number_of_classes=2)
